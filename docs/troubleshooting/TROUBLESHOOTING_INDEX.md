...
| [2026-09-19 启动器打开网页应用闪退](2026-09-19-launcher-open-duplicate-registry-crash.md) | 排查「点启动器里的网页应用宿主立刻没了 / EXC_BREAKPOINT + `uniqueKeysWithValues` / `refreshRegistry`」、或改运行时注册表 / 单实例锁。不读会在同应用出现两份运行状态时用强制唯一字典把主程序打死。 |
| [2026-09-18 长页面把窗口卡死](2026-09-18-long-page-webview-jank.md) | 排查「网页应用内容一多就打字卡、滚动卡、窗口不可用」（对话、信息流都算），或准备给容器加显示/GPU/卸页/「长列表卸屏」之前。不读会做成某个网站的定点补丁、用卸页破坏秒开，或把已否决的屏外跳过绘制脚本加回去。 |
